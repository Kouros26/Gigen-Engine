...
TEST_CASE("Quaternion", "[.all][Quaternion]")
{
	...
	SECTION("Transformation")
	{
		...
		SECTION("Rotate")
		{
			...
			SECTION("Around Axis")
			{
				lm::FQuat rotated = FQuat::Rotate(origin, lm::FVec3{ 1.f, 0.f, 0.f });

				glm::quat qv = glm::quat(0.f, 1.f, 0.f, 0.f);
				glm::quat rotatedGlm = originGlm * qv * glm::inverse(originGlm);

				CHECK_QUAT(rotated, rotatedGlm);
			}
			SECTION("Around Quat")
			{
				lm::FQuat rotated = FQuat::Rotate(origin, rotation);

				glm::quat rotatedGlm = originGlm * rotationGlm * glm::inverse(originGlm);

				CHECK_QUAT(rotated, rotatedGlm);
			}
			SECTION("Around Axis and Angle")
			{
				lm::FQuat rotated = FQuat::Rotate(origin, lm::FVec3{ 1.f, 0.f, 0.f }, 45.f);
