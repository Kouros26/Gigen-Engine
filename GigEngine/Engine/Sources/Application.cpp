...
void Application::CreateGameObjects()
{
	...
	chest->SetModel("Engine/Models/chest.obj");
	chest->SetTexture("Engine/Textures/test.png");

	GameObject* car = GameObjectManager::CreateGameObject("car", { -5, 10, 10 }, { 0 }, { 1 });
	car->SetModel("Engine/Models/Car.fbx");
	...
	ground->SetModel("Engine/Models/Basics/Cube.FBX");
	...
	TH->SetModel("Engine/Models/Thierry-Henri.obj");
	TH->CreateSphereRigidBody(2, { 0.05f }, 10.0f);
	TH->GetRigidBody()->SetGravityEnabled(false);
	TH->AddComponent<GigScripting::Behaviour>("test");
